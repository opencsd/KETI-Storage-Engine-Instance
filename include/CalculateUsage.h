...
void CalculateStart(){
    ...
    double StorageJoin = 13662.5886;
    double StorageOrderby = 18966.8386;
    double StorageGroupby = 15179.1723;
...
void CalculateEnd(){
    ...
    double cpuUsage = (static_cast<double>(cpuTimeUsed) / sysconf(_SC_CLK_TCK)) / (duration.count()) * 100.0;
...
//17
    ...
    std::cout << "RX bytes used Eno: " << rxBytesUsedEno << std::endl;
    std::cout << "RX bytes used Lo : " << rxBytesUsedLo << std::endl;
    std::cout << "TX bytes used Eno: " << txBytesUsedEno << std::endl;
    ...
    std::cout<<"Host CPU Usage During Query Execution -> "<<cpuUsagePercentage<<" %"<<std::endl;
    ...
    // std::cout<<"\nSimulated Result..."<<std::endl;
    // std::cout<<"Query Done... Time -> "<<(duration.count()/1000.0) * 1.025<<" s"<<std::endl;
    // std::cout<<"Host CPU Usage During Query Execution -> "<<(cpuUsagePercentage + 4.7)*1.025<<" %"<<std::endl;
    // std::cout<<"Energy Usage during query execution   -> "<<powerUsage*1.025<<" W"<<std::endl;
    // std::cout<<"Network Usage during query execution  -> "<<totalBytesUsed/1000000.0 * 1.025<<" MB"<<std::endl;
    return;
}