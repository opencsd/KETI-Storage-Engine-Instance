...
void MergeQueryManager::RunSnippetWork(){
    ...
    if(isOrderby){
        ...
        //Save "result_table"
        if(isLimit){
            BufferManager::SaveTableData(snippet.query_id(),snippet.table_alias(),having_table,snippet.limit().offset(),snippet.limit().length());
        }else{
            BufferManager::SaveTableData(snippet.query_id(),snippet.table_alias(),having_table,0,having_table.row_count);
        } 
    }
...
void MergeQueryManager::DependencyInnerJoin(){
    ...
    TableData dependency_subquery_result_table;
    for (auto it = right_table.table_data.begin(); it != right_table.table_data.end(); it++){
        dependency_subquery_result_table.table_data[(*it).first].type = (*it).second.type;
    }

    vector<int> equal_join_index;//join조건 중 equal 조인의 인덱스
    for(int f = 0; f < snippet.dependency().dependency_filter_size(); f++){
        if(snippet.dependency().dependency_filter(f).operator_() == Snippet_Filter_OperType::Snippet_Filter_OperType_KETI_ET){
            equal_join_index.push_back(f);
        }
    }

    right_is_smaller = true;//dependency subquery table로 해시테이블 생성
    equal_join_exist = (equal_join_index.size() != 0)? true : false;

    createHashTable(equal_join_index);

    string lv_column = snippet.table_filter(0).lv().value(0);
    int lv_type = left_table.table_data[lv_column].type;

    if(lv_type == TYPE_STRING){
        for(int r1=0; r1<left_table.row_count; r1++){
            string lv_value = left_table.table_data[lv_column].strvec[r1];

            for(int r2=0; r2<right_table.row_count; r2++){
                bool passed = true;

                for(int f=0; f<snippet.dependency().dependency_filter_size(); f++){
                    string dependency_lv_column = snippet.dependency().dependency_filter(f).lv().value(0);
                    string dependency_rv_column = snippet.dependency().dependency_filter(f).rv().value(0);
                    int oper = snippet.dependency().dependency_filter(f).operator_();
                    ...
                    if(dependency_lv_type == TYPE_STRING){
                        ...
                        float dependency_rv_value = right_table.table_data[dependency_rv_column].floatvec[r2];
                        passed = compareByOperator(oper,dependency_lv_value,dependency_rv_value);
                    }
                ...
                if(passed){ 
                    for (auto it = right_table.table_data.begin(); it != right_table.table_data.end(); it++){
                        switch((*it).second.type){
                        case TYPE_INT:
                            dependency_subquery_result_table.table_data[(*it).first].intvec.push_back((*it).second.intvec[r2]);
                            break;
                        case TYPE_STRING:
                            dependency_subquery_result_table.table_data[(*it).first].strvec.push_back((*it).second.strvec[r2]);
                            break;
                        case TYPE_FLOAT:
                            dependency_subquery_result_table.table_data[(*it).first].floatvec.push_back((*it).second.floatvec[r2]);
                            break;
                        }
                        dependency_subquery_result_table.table_data[(*it).first].isnull.push_back((*it).second.isnull[r2]);
                        dependency_subquery_result_table.table_data[(*it).first].row_count++;
                    }
                    dependency_subquery_result_table.row_count++;
                }
        ...
        for(int r1=0; r1<left_table.row_count; r1++){
            int lv_value = left_table.table_data[lv_column].intvec[r1];
    ...
    }else if(lv_type == TYPE_FLOAT){
        for(int r1=0; r1<left_table.row_count; r1++){
            float lv_value = left_table.table_data[lv_column].floatvec[r1];
...
void MergeQueryManager::Having(){
    // for(int r=0; r<result_table.row_count; r++){
    //     bool passed = true;

    //     for(int h=0; h<snippet.having_size(); h++){
    //         if(h%2 == 1){
    //             if(snippet.having(h).operator_() == Snippet_Filter_OperType::Snippet_Filter_OperType_KETI_AND){
    //                 if(passed == false) break;
    //             }else{
    //                 KETILOG::ERRORLOG(LOGTAG,"Filtering: " + snippet.having(h).operator_());
    //             }
    //         }else{
    //             string left_col = snippet.having(h).lv().value(0);

    //             switch(result_table.table_data[left_col].type){
    //             case TYPE_STRING:{
    //                 string lv = trim(result_table.table_data[left_col].strvec[r]);
    //                 string rv = snippet.having(h).rv().value(0);
    //                 passed = compareByOperator(snippet.having(h).operator_(), lv, rv);
    //                 break;
    //             }case TYPE_FLOAT:{
    //                 float lv = result_table.table_data[left_col].floatvec[r];
    //                 float rv = stof(snippet.having(h).rv().value(0));
    //                 passed = compareByOperator(snippet.having(h).operator_(), lv, rv);
    //                 break;
    //             }case TYPE_INT:{
    //                 int lv = result_table.table_data[left_col].intvec[r];
    //                 int rv = stoi(snippet.having(h).rv().value(0));
    //                 passed = compareByOperator(snippet.having(h).operator_(), lv, rv);
    //                 break;
    //             }default:
    //                 KETILOG::ERRORLOG(LOGTAG,"Filtering (d)");
    ...
    //     if(passed){ 
    //         for (auto it = result_table.table_data.begin(); it != result_table.table_data.end(); it++){
    //             switch((*it).second.type){
    //             case TYPE_INT:
    //                 having_table.table_data[(*it).first].intvec.push_back((*it).second.intvec[r]);
    //                 break;
    //             case TYPE_STRING:
    //                 having_table.table_data[(*it).first].strvec.push_back((*it).second.strvec[r]);
    //                 break;
    //             case TYPE_FLOAT:
    //                 having_table.table_data[(*it).first].floatvec.push_back((*it).second.floatvec[r]);
    //                 break;
    //             }
    //             having_table.table_data[(*it).first].isnull.push_back((*it).second.isnull[r]);
    //             having_table.table_data[(*it).first].row_count++;
    //         }
    //         having_table.row_count++;
    //     }
    // }
}
