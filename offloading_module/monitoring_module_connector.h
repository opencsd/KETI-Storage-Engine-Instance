...
class MonitoringModuleConnector {
	public:
		...
		SnippetMetaData GetSnippetMetaData(string dbname, string tname, ScanInfo scanInfo, map<string,string> sst_csd_map) {
			...
			for(int i=0; i<request.scan_info().sst_csd_map_size(); i++){
				string sst_name = request.scan_info().sst_csd_map(i).sst_name();
				request.mutable_scan_info()->mutable_sst_csd_map(i)->clear_csd_list();
				request.mutable_scan_info()->mutable_sst_csd_map(i)->add_csd_list(sst_csd_map[sst_name]);
			}
